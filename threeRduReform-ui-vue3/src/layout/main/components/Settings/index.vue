<template>
  <el-drawer v-model="showSettings" :withHeader="false" :lock-scroll="false" direction="rtl" size="300px">
    ...
    <div class="drawer-item">
      <span>导航菜单风格</span>
      <span class="comp-style">
        <el-button :type="menuStyle === 'rounded' ? 'primary' : 'default'" size="small" @click="handleMenuStyle('rounded')">圆润</el-button>
        <el-button :type="menuStyle === 'plain' ? 'primary' : 'default'" size="small" @click="handleMenuStyle('plain')">朴素</el-button>
      </span>
    </div>
...
<script setup lang="ts">
  ...
  const menuStyle = ref<number>(settingsStore.menuStyle);
  ...
  function handleMenuStyle(val) {
    settingsStore.menuStyle = val;
    menuStyle.value = val;
  }
  ...
  function saveSetting(): void {
    ...
    let layoutSetting = {
      ...
      menuStyle: storeSettings.value.menuStyle,
    };
