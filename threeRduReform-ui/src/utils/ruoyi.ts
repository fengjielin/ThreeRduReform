...
/** parseTree 回调函数类型 */
export type ParseTreeCallback = (node: TreeNode, index: number, parent: TreeNode | null) => void;

/* 递归遍历树形结构数据 */
export function parseTree(treeNodes: TreeNode[], callback: ParseTreeCallback = () => {}, level: number = 0, parent: TreeNode | null = null): void {
  if (!treeNodes || !treeNodes.length) return;
  for (let i = 0; i < treeNodes.length; i++) {
    if (typeof level === 'number') treeNodes[i].level = level;
    callback && callback(treeNodes[i], i, parent);
    const _children = treeNodes[i].children;
    if (_children && _children.length > 0) {
      parseTree(_children, callback, level + 1, treeNodes[i]);
    }
  }
}
...
export function isValidUrl(url: string): boolean {
  return /^(https?|ftp):\/\/([a-zA-Z0-9.-]+(:[a-zA-Z0-9.&%$-]+)*@)*((25[0-5]|2[0-4][0-9]|1[0-9]{2}|[1-9][0-9]?)(\.(25[0-5]|2[0-4][0-9]|1[0-9]{2}|[1-9][0-9]?)){2}|([a-zA-Z0-9-]+\.)*[a-zA-Z0-9-]+\.(com|edu|gov|int|mil|net|org|biz|arpa|info|name|pro|aero|coop|museum|mobi|travel|[a-z]{2}))(:[0-9]+)*(\/($|[a-zA-Z0-9.,?'\\+&%$#=~_-]+))*$/.test(
    url,
  );
}
