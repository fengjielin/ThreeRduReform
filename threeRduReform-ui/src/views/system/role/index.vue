...
<script setup lang="ts" name="Role">
  ...
  import { parseTree } from '@/utils/ruoyi';
  ...
  const computedMenuOptions = computed(() => {
    if (menuList.value && menuList.value.length > 0) {
      parseTree(menuOptions.value, (item) => {
        let menu = menuList.value.find((menu) => menu.menuId === item.id);
        if (menu) {
          console.log(item, menu);
          item.visible = menu.visible;
          item.status = menu.status;

          if (menu.visible == '1' && item.label.indexOf('[已隐藏]') == -1) {
            item.label = item.label + '[已隐藏]';
          }
          if (menu.status == '1' && item.label.indexOf('[已停用]') == -1) {
            item.label = item.label + '[已停用]';
          }
        }
      });
